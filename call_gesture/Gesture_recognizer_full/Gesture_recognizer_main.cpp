...
int main(int argc, char** argv)
{
	...
	if (mode == "train")
	{
		...
		int scanner_wide = 100;
		int scanner_high = 200;
	...
	if (mode == "test")
	{
		...
		models_name = (string**)malloc(sizeof(string*) * model_num);
		// 存储模型文件名
		for (int i = 0; i < model_num; i++)
		{
			const std::string name_temp = argv[5 + i];
			models_name[i] = (string*)malloc(sizeof(char)*128);
			memset(models_name[i], 0, sizeof(char) * 128);
