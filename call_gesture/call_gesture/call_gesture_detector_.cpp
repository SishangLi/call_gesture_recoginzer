...
{  
    ...
    try
    {
        ...
        dlib::array<array2d<unsigned char> > images_train, images_test, images_test_union;
        std::vector<std::vector<rectangle> > face_boxes_train, face_boxes_test, face_boxes_test_union;
        ...
        load_image_dataset(images_train, face_boxes_train, faces_directory + "/training_3.xml");
        load_image_dataset(images_test, face_boxes_test, faces_directory + "/testing_3.xml");
		load_image_dataset(images_test_union, face_boxes_test_union, faces_directory + "/testing.xml");
		...
		// upsample_image_dataset<pyramid_down<2> >(images_test_union, face_boxes_test_union);
		...
		// 在跑出无法命中目标框的异常时，首先检查标注，必须比例为1
		// 左手手势训练数据
        // scanner.set_detection_window_size(90,90); //training_1
		// scanner.set_detection_window_size(110, 110); //training_2
		 scanner.set_detection_window_size(100, 200); //training_3
        ...
        // iteration so you can see how close it is to finishing the training.  
         trainer.set_epsilon(0.01);//training_1
		

		// 误检loss
		// 左手手势训练数据
		// trainer.set_loss_per_missed_target(2.7);//training_1         
		// trainer.set_loss_per_missed_target(2.0); //training_2     
		// trainer.set_loss_per_missed_target(0.5); //training_3  
		// 右手手势训练数据
		// trainer.set_loss_per_missed_target(3);//training_1         
		// trainer.set_loss_per_missed_target(2.5); //training_2    
		 trainer.set_loss_per_missed_target(5); //training_3  
		// 移除不能识别的矩形框
		 // 左手手势训练数据
		remove_unobtainable_rectangles(trainer, images_train, face_boxes_train); //training_3——need
		// 右手手势训练数据
		...
		serialize("Gesture_detector_right_3.svm") << detector;
		...
		//在联合测试集上做预测
		image_window win2;
		for (unsigned long i = 0; i < images_test_union.size(); ++i)
		{
			// Run the detector and get the face detections.
			std::vector<rectangle> dets = detector(images_test_union[i]);
			win2.clear_overlay();
			win2.set_image(images_test_union[i]);
		...
		for (unsigned long i = 0; i < images_train.size(); i++)
		{
			...
			win3.clear_overlay();
			win3.set_image(images_train[i]);
			win3.add_overlay(dets, rgb_pixel(255, 0, 0));
			cout << "Hit enter to process the next image..." << endl;
			cin.get();
		}
		...
		deserialize("Gesture_detector_right_3.svm") >> detector2;
